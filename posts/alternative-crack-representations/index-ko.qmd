---
title: "균열을 표현하려면 반드시 변위공간의 확충이 필요한가?"
subtitle: "확충된 근사공간에서 phantom node, cracking particle, peridynamics, phase field까지"
author: "Goangseup Zi"
date: "2026-09-04"
categories:
  - Research Explained
  ...
  - Phase Field
  - Peridynamics
description: "재료가 연속성을 잃었다는 정보를 수치해석의 어디에 담을 것인가라는 질문을 중심으로 여러 파괴해석 기법을 역학적으로 비교한다."
lang: ko
toc: true
toc-depth: 2
---
...
**계산파괴역학 시리즈:** [← 전체 보기](../computational-fracture-overview/index-ko.qmd) · [← 다중균열](../multiple-cracks-fracture-network/index-ko.qmd) · 7편 중 5편

앞의 글들에서는 하나의 생각을 계속 따라왔다.

균열이 생기면 보통의 연속 근사로는 표현할 수 없는 운동학이 나타난다. **확장유한요소법(XFEM)**과 **확장무요소법(XEFG)**은 이때 빠져 있는 불연속 정보를 변위공간을 확충한다.

그러면 자연스럽게 이런 질문이 생긴다.

> **균열을 표현하려면 반드시 변위공간을 확충(enrichment)해야 하는가?**

그렇지는 않다.

하지만 어떤 파괴해석 기법이 가장 좋은가를 묻는 것보다 더 중요한 질문이 있다.

> **“여기에 균열이 있다”는 정보를 수치해석의 어디에 저장할 것인가?**

파괴해석 기법들의 차이는 상당 부분 이 질문에 대한 서로 다른 답으로 볼 수 있다.
...
겹쳐진 요소 영역으로 불연속을 표현
        ↓
cracking particle
입자의 support / 가시성(visibility) 변화로 불연속을 표현
        ↓
peridynamics
비국소 상호작용 자체가 불연속을 허용
        ↓
phase field
날카로운 균열을 정규화된 파괴장으로 대체
```
...
## 먼저 물리가 요구하는 것이 무엇인지 보자

수치기법을 보기 전에 파괴가 역학적으로 무엇을 요구하는지부터 생각해 보자.

균열이 생기기 전에는 내부의 어떤 면을 사이에 두더라도 변위장은 연속이다.

$$
\llbracket \mathbf{u} \rrbracket = \mathbf{0}.
$$

균열이 생긴 뒤에는 양쪽 면이 서로 독립적으로 움직일 수 있다.
...
> **역학적으로 연속성이 사라져야 하는 곳에서 근사함수가 어떻게 실제로 연속성을 잃게 할 것인가?**

XFEM은 근사공간을 확충함으로써 답했다. 이후의 방법들은 같은 정보를 다른 곳으로 옮겼다.
...
XFEM은 이 근사에 불연속을 표현할 수 있는 함수를 추가한다. 가장 단순한 변위 점프를 표현하기 위한 확충 형태는
...
물론 변위공간의 확충을 도입하면 구현에서 새로운 문제가 생긴다. 어느 절점에서 변위공간을 확충할 것인가, 일부만 잘린 요소를 어떻게 적분할 것인가, 균열선단은 어떻게 표현할 것인가, 추가 자유도는 어떻게 조립할 것인가, 동적 문제의 질량행렬은 어떻게 다룰 것인가, 복잡한 균열 위상은 어떻게 관리할 것인가 등의 문제다.

이런 문제들이 다른 표현법을 찾게 만든 이유 중 하나였다.

## Phantom node: 이번에는 이산화된 영역을 바꾼다

Phantom-node 방법도 날카로운 균열(sharp crack)을 유지한다. 다만 균열 양쪽을 표현하는 방식이 다르다.
...
![표준 XFEM 표현과 phantom-node 표현의 비교. 두 방법 모두 날카로운 균열을 가로지르는 변위 점프를 표현하지만, 추가적인 운동학적 자유도를 저장하는 방식이 다르다. Rabczuk et al. (2008), Fig. 2를 바탕으로 재구성.](Fig01.png){#fig-phantom-xfem fig-align="center" width="84%"}

그림 @fig-phantom-xfem에서 보아야 할 것은 방법의 이름이 아니다.

물리는 바뀌지 않았다.

균열은 여전히 날카로운 변위 불연속이다.

달라진 것은 **근사공간을 관리하는 방식**, 말하자면 bookkeeping이다.

## 왜 명시적인 확충 자유도에서 벗어나려 했을까?
...
여기서 계산역학의 조금 더 일반적인 교훈을 얻을 수 있다.
...
균열이 요소 내부에서 끝나면 선단에서 변위 점프가 적절히 0으로 닫혀야 한다. 그래서 우리의 phantom-node 연구에서도 임의의 점성균열을 위한 crack-tip element를 도입했다.

앞서 XFEM에서 만났던 것과 똑같은 물리적 요구다.
...
무요소법은 고정된 요소 연결성 대신 입자의 support domain으로 근사를 구성하기 때문에 또 다른 가능성을 제공한다.
...
이제 그 정보는 주로 추가 확충 자유도에 들어 있는 것이 아니라 **입자 사이의 support 관계**에 들어 있다.
...
## 날카로운 균열을 쓰는 한 기하학은 추적해야 한다

XFEM, XEFG, phantom-node, cracking-particle 방법은 구현상 상당히 다르다.

하지만 중요한 공통점이 있다.

모두 식별 가능한 **날카로운 불연속(sharp discontinuity)**을 유지한다.
...
> **아예 날카로운 균열면을 명시적으로 표현하지 않으면 어떨까?**

## Peridynamics: 근사법이 아니라 지배방정식의 기술방식을 바꾼다

Peridynamics는 다른 방향에서 파괴문제에 접근한다.

고전적인 국부 연속체역학은 변위장의 공간미분을 사용한다. 그런데 변위 불연속이 생기면 바로 그 지점에서 고전적인 의미의 미분이 문제가 된다.
...
우리의 열가소성 재료 파괴 연구에서는 3차원 열-역학 파괴를 위해 non-ordinary state-based peridynamic formulation을 사용했다.

이것은 XFEM에서 phantom node로 옮겨가는 것보다 더 큰 변화다.

균열 정보가 이산 근사공간에서 **진화하는 비국소 재료 상호작용의 상태**로 이동했기 때문이다.

## 변위공간의 확충이 없다고 파괴모델링이 없어지는 것은 아니다
...
## Phase field: 날카로운 균열을 하나의 장으로 바꾼다

Phase-field fracture는 날카로운 균열이라는 그림에서 가장 분명하게 벗어나는 방법 중 하나다.

균열면 $\Gamma$를 명시적으로 기술하는 대신 스칼라장 $\phi(\mathbf{x})$를 도입한다.

우리의 나노복합재 연구에서 사용한 phase-field formulation에서는
...
따라서 균열은 두께가 0인 기하학적 면이 아니라 **확산된 파괴장(diffuse fracture field)**으로 표현된다.
...
날카로운 균열에 대한 기하학적 표면적분을 좁은 영역에 분포한 장의 체적적분으로 바꾸었다.
...
그림 @fig-phase-field의 핵심적인 교환관계는 분명하다.
...
> 물체 전체에서 파괴장의 값은 얼마인가?

를 푼다.

균열의 위상을 같은 방식으로 명시적으로 갱신하지 않는다. **균열 형상이 장의 해로부터 나타난다.**
...
하지만 같은 연구는 길이척도와 재료모델이 왜 중요한지도 보여준다.

Interphase의 두께와 물성이 인장강도, 전체 에너지방출률, 소산된 파괴에너지에 영향을 주었다.
...
- 확산 균열 부근의 충분한 해상도,
- 추가적인 장 방정식,
- 두께 0인 정확한 면 대신 유한 폭의 손상영역에 대한 해석.
...
그래서 이 글을 시간순으로 발전한 방법의 사다리로 읽으면 안 된다.
...
어떤 방법이 가장 최신인가보다 이런 질문들이 훨씬 유용하다.
...
**같은 재료 연속성의 상실을 표현하는 정보를 수치해석의 서로 다른 곳에 저장한 것이다.**
...
균열개구 $\llbracket\mathbf{u}\rrbracket$와 점성 traction $\mathbf{t}$가 국부적인 파괴 일을 전달한다.
...
> **균열은 특정한 하나의 수치기법을 요구하지 않는다. 물리가 요구하는 연속성의 상실을 표현할 수 있는 수치표현을 요구한다.**
...
[← **이전: 여러 균열은 어떻게 분기하고 만나 파괴 네트워크를 만드는가?**](../multiple-cracks-fracture-network/index-ko.qmd)
