...
**Computational Fracture Series:** [← Overview](../computational-fracture-overview/) · [← XFEM](../xfem-crack-through-mesh/) · Article 3 of 7 · [Next: Multiple cracks →](../multiple-cracks-fracture-network/)
...
![A crack interacts with meshfree domains of influence differently depending on whether a support is uncut, completely cut, or contains the crack tip. The figure should be read as a kinematic map: enrichment is activated according to how the discontinuity intersects the support, not according to element connectivity. Adapted from Rabczuk and Zi (2007).](Fig01.png){#fig-xefg-support fig-align="center" width="78%"}

Figure @fig-xefg-support shows the key geometric difference between XFEM and XEFG. In XFEM, we usually ask how a crack cuts an element and its associated nodal supports. In XEFG, the corresponding question is how the crack cuts overlapping **domains of influence**. The physical discontinuity is the same, but the bookkeeping of the approximation changes.
...
![Cohesive fracture can be represented in XEFG without imposing the classical LEFM crack-tip branch enrichment. The important point is not the absence of a particular mathematical function by itself, but that the approximation is designed around the regularized cohesive crack-tip mechanics. Adapted from Zi, Rabczuk, and Wall (2007).](Fig02.png){#fig-xefg-no-branch fig-align="center" width="78%"}

Figure @fig-xefg-no-branch illustrates an important modeling choice. If the cohesive law already regularizes the crack-tip response, inserting a classical singular asymptotic field is not automatically necessary. The enrichment should supply the **missing kinematics**, not mathematical structure that the assumed fracture model does not require.
...
![Three-dimensional XEFG allows an evolving crack to be represented as a surface bounded by a moving crack front. Curvature, propagation, branching, and junctions therefore become problems of both fracture mechanics and evolving geometry. Adapted from the three-dimensional XEFG work of Rabczuk, Bordas, and Zi (2007).](Fig03.png){#fig-xefg-3d fig-align="center" width="82%"}

Figure @fig-xefg-3d shows why three-dimensional fracture is qualitatively harder than its two-dimensional counterpart. The numerical method is no longer tracking only a crack-tip point. It must maintain a physically meaningful **surface and crack front** while that geometry evolves through the background approximation.
...
[← **Previous: How Does XFEM Let a Crack Cut Through the Mesh?**](../xfem-crack-through-mesh/)

[**Series overview: Why Is Computational Fracture So Difficult?**](../computational-fracture-overview/)

[**Next: How Do Multiple Cracks Branch, Join, and Form a Fracture Network? →**](../multiple-cracks-fracture-network/)
