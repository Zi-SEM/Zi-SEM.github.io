...
keywords:
  ...
  - "level set method"
  - "remeshing"
  - "mesh independent crack growth"
  - "multiple crack growth"
  - "superimposed finite element method"
description: "A mechanics-oriented explanation of XFEM: partition-of-unity enrichment, displacement jumps, cohesive crack-tip treatment, evolving crack geometry, and dynamic fracture without remeshing."
toc: true
toc-depth: 2
---
...
**Computational Fracture Series:** [← Overview](../computational-fracture-overview/) · Article 2 of 7 · [Next: XEFG →](../xefg-why-meshfree/)
...
![A cohesive crack must terminate inside the crack-tip element while its displacement jump closes toward the tip. This is the kinematic problem addressed by the new XFEM crack-tip formulation. Source: Zi and Belytschko (2003); replace this placeholder with the selected original-paper figure.](Fig01.png){#fig-cohesive-tip fig-align="center" width="72%"}

Figure @fig-cohesive-tip should be read from **kinematics to consequence**: the crack is allowed to cut the element, but the discontinuity cannot simply continue through the tip. The crack-tip treatment supplies the missing closure condition without forcing the mesh boundary to coincide with the crack.
...
→ additional crack-tip resolution where it is useful
```

The important idea is not to refine the entire global mesh merely because one small region contains difficult fracture mechanics.

![The combined extended and superimposed finite element method separates the global structural discretization from a local crack-tip discretization. Source: Lee et al. (2004); replace this placeholder with the selected original-paper figure.](Fig03.png){#fig-superimposed fig-align="center" width="74%"}

Figure @fig-superimposed illustrates a broader computational strategy: **put numerical resolution where the physics requires it, rather than making the whole discretization pay the same cost.**
...
![Dynamic fracture represented with a discontinuous enriched approximation. The crack evolves through the background discretization rather than being constrained to element boundaries. Source: Belytschko, Chen, Xu, and Zi (2003), Fig. 12.](Fig02.png){#fig-dynamic-fracture fig-align="center" width="80%"}

Figure @fig-dynamic-fracture is important for what it demonstrates numerically: the physical discontinuity evolves while the background discretization remains a computational scaffold. The crack path is therefore governed by the fracture formulation rather than by a requirement that it follow element edges.
...
[**Next: Why Extend XFEM into a Meshfree Method? →**](../xefg-why-meshfree/)
